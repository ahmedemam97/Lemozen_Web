...
/*    if(tokenExist){*/
    ...
    window.getPageInfo('PrivateTours/GetAll',StartPage)
    ...
    $(document).on('click','#saveBtn',function(){
        ...
        if(model.Name.trim() == ""){
            window.error('ادخل اسم الرحلة')
            return
        }
        if(model.Hour <= 0){
            window.error('ادخل   الساعة')
            return
        }
        console.log(model)
        if(model.Id == 0){
            window.sendToserver('PrivateTours/Create',model,callBack)
        }else {
            window.sendToserver('PrivateTours/Update',model,callBack)
        }
    })
    //
    ...
    $(document).on('click','#deleteBtn',function(){
        ...
        function confirm(){
        window.DeleteFromServer(`PrivateTours/Delete?id=${id}`,DeleteCallBack)
        }
    })
    ////////
